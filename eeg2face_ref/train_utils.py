...
def experiment_dir(config: Config) -> Path:
    return config.train.results_dir / config.data.dataset.lower() / "reference"
...
def run_training(config: Config) -> dict[str, Any]:
    ...
    train_loader, _, test_loader, split_info = create_dataloaders(config)
    ...
    model = EEGToFaceLatent(
        ...
        num_sample_identities=int(split_info["unique_aligned_samples"]),
    ).to(device)
    ...
    for epoch in range(1, config.train.epochs + 1):
        ...
        test_metrics = run_epoch(model, autoencoder, test_loader, device, config, None, desc=f"test {epoch:03d}")
        scheduler.step()
        history.append({"epoch": epoch, **_prefix("train", train_metrics), **_prefix("test", test_metrics)})
        LOGGER.info(
            "Epoch %03d/%03d train_loss=%.6f train_mse=%.6f train_psnr=%.3f train_ssim=%.6f "
            "test_loss=%.6f test_mse=%.6f test_psnr=%.3f test_ssim=%.6f",
            epoch,
            config.train.epochs,
            train_metrics["loss"],
            ...
            test_metrics.get("psnr", float("nan")),
            test_metrics.get("ssim", float("nan")),
        )

        current_score = test_metrics.get(best_metric)
        if current_score is None:
            raise KeyError(f"Unknown best metric: {best_metric}. Available metrics: {sorted(test_metrics)}")
        if is_better(float(current_score), best_score, best_metric):
            best_score = float(current_score)
            best_epoch = epoch
            best_state = deepcopy(model.state_dict())
            save_checkpoint(output_dir / "best_eeg2face.pt", model, config, split_info, best_epoch, best_score)
            LOGGER.info("Saved best_eeg2face.pt by test_%s=%.6f at epoch %03d.", best_metric, best_score, best_epoch)
    ...
    results = {
        **split_info,
        ...
        "best_metric": f"test_{best_metric}",
...
def run_epoch(
    ...
    if save_examples is not None and saved_targets is not None and saved_predictions is not None:
        save_reconstruction_grid(
            ...
            saved_predictions,
            save_examples,
            f"EEG-to-face reference, SSIM={metrics.get('ssim', float('nan')):.4f}",
        )
    return metrics
