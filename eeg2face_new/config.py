...
class DataConfig:
    ...
    emoji_size: int = DATASET_PRESETS["EAV"].emoji_size
    eeg_window_seconds: float = 1.0
    split_mode: str = "trial"
...
class ModelConfig:
    ...
    use_trial_frame_embedding: bool = False
    trial_embedding_dim: int = 512
    frame_embedding_dim: int = 64
    use_sample_embedding: bool = False
    sample_embedding_dim: int = 0
...
class TrainConfig:
    ...
    device: str = "cuda:0"
    results_dir: Path = Path("./eeg2face_clean_results")
    save_every: int = 0
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Clean EEG-to-face training with a frozen Face2Face AutoencoderKL tail.")
    parser.add_argument("--dataset", choices=sorted(DATASET_PRESETS), default="EAV", choices=["EAV", "MMER"])
    parser.add_argument("--eeg-dir", type=Path, default=None)
    parser.add_argument("--face-root", type=Path, default=None)
    parser.add_argument("--subject-ids", default=None)
    ...
    parser.add_argument("--split-mode", choices=["trial", "subject"], default="trial")
    ...
    parser.add_argument("--autoencoder", type=Path, default=Path("/home/devuser/hjj/seeemotion/models/AutoencoderKL.pth"))
    ...
    parser.add_argument("--use-subject-embedding", action="store_true")
    parser.add_argument("--subject-embedding-dim", type=int, default=32)
    parser.add_argument("--use-trial-frame-embedding", action="store_true")
    ...
    parser.add_argument("--results-dir", type=Path, default=Path("./eeg2face_clean_results"))
...
def build_config(args: argparse.Namespace) -> Config:
    ...
    return Config(
        ...
        model=ModelConfig(
            ...
            use_trial_frame_embedding=args.use_trial_frame_embedding,
